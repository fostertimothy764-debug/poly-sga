  ...
  return (
    <div className="container-page py-10 sm:py-14 animate-pulse">
      <div className="mb-10 pb-6 border-b border-ink-200">
        ...
        <div className="h-10 w-60 rounded bg-ink-200 mb-3" />
        <div className="h-4 w-full max-w-2xl rounded bg-ink-100" />
      </div>
      <div className="h-32 rounded-2xl border border-ink-200 bg-white mb-10" />
      <div className="space-y-3">
        {[0, 1, 2].map((i) => (
          <div key={i} className="h-24 rounded-2xl border border-ink-200 bg-white" />
        ))}
      </div>
    </div>
  );
